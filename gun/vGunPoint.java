...
public class vGunPoint
{
	public static VirtualBullet headOn(TimedPoint source, Enemy target, double BULLET_POWER)
    {
		return new VirtualBullet(new Point2D.Double(target.getX(), target.getY()),
				(long) (Point2D.distance(target.getX(), target.getY(), source.getX(), source.getY())/Rules.getBulletSpeed(BULLET_POWER))+source.getTime(),
				0);

    } 
    
    //LINEAR PREDICTION (MY CODE & MATH)
    public static VirtualBullet linearPredictionFire(TimedPoint source, Enemy target, double BULLET_POWER)
    {
    	double time = -1;
    	double endX = 0;
    	double endY = 0;
    	...
    	return new VirtualBullet(new Point2D.Double(endX, endY),
				(long) time+source.getTime(),
				1);
    }

    public static VirtualBullet constantTurnPredict(TimedPoint source, Enemy target,  double BULLET_POWER)
    {
    	...
    	Point2D.Double targetCoord = Util.limitCoordinateToMap(new Point2D.Double(relX + selfX, relY +selfY));
     	...
     	return new VirtualBullet(targetCoord,
				(long) deltaTime+source.getTime(),
				2);
    }
    
    public static VirtualBullet averageMovementGun(TimedPoint source, Enemy target, double BULLET_POWER)
    {
