...
/*		try {
      ...
      /*      	if (getDistanceRemaining() == 0)
            ...
            if (moveWait == 0)
            {
	            switch (moveMode)
	            {
		            ...
		            case 3 :
		            	if (getOthers() > 1)
		            		destinationPoints = Move.fourCornersLoose(this);
		            	else 
		            	{
		            		moveMode = 4;
		            		destinationXY = null;
		            ...
		            case 4 : 
		            	if (cTargetEnemy != null)
		            		destinationPoints = Move.circularPointDistribution(this, cTargetEnemy);
		            	break;
		            case 5 : //CURRENT
		            	if (getOthers() > 4)
		            	{
		            		final double HYPOT = 150;
		            		destinationPoints = Move.circleDistribution_NeverClosest2(this, closestDistCircles, currentEnemies, HYPOT);
		            	}
		            	else 
		            	{
		            		moveMode = 9000;
	...
	{
		Move.circularAwayPointDistribution(this, cTargetEnemy, 150);
		moveWait = 16;
	}
    ...
    public void onPaint(Graphics2D g) 
    {
    	...
    	if (showMovement && destinations != null)
    	{
			...
			Point2D.Double[] destinations = Move.generatePointsCircular(new Point2D.Double(getX(), getY()), 16, 150);
	    	//paths = Move.generatePaths(this, destinations);
